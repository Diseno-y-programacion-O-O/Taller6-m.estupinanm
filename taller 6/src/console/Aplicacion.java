...
import logic.ValorTotalExcedido;
...
public class Aplicacion {
	...
	private void mostrarMenuRestaurante() {
		...
		index = 1;
		System.out.println("\n\nBebidas\n");
		System.out.format(formatPrint, "#", "Bebida", "Valor");
		System.out.println();
		...
		for (Ingrediente ingrediente : ingredientes) {
			System.out.format(formatPrint, Integer.toString(index) + "- ", ingrediente.getNombre(),
	...
	private void agregarElementoAPedido() {
		if (restaurante.getPedidoEnCurso() == null) {
			System.out.println("No se ha iniciado ningún pedido.");
		} else {
			Producto producto;
			...
			try {
				...
				System.out.println("\nSe agregó " + producto.getNombre() + " al pedido");
			} catch (ValorTotalExcedido e) {
				System.out.println(
						"\nNo se puede agregar el producto al pedido. El valor total excede el límite permitido.");
			}
		}
	}
	...
	private void cerrarPedido() {
		if (restaurante.getPedidoEnCurso() == null) {
			...
			int pedidoSimilar = restaurante.cerrarYGuardarPedido();
			if (pedidoSimilar > 0) {
				System.out.println("Se han hecho " + Integer.toString(pedidoSimilar) + " Pedidos similares");
			} else {
				System.out.println("No se han hecho pedidos similares");
			}
		}
	}
